...
# Model parameters ----------------------------------------------------------------------------------------------------#
hidd = 4                            # Hidden channels parameter for baseline model                                     #
model_type = "baseline_simple"      # Model type, can be # TODO: Complete model types                                  #
# Training parameters -------------------------------------------------------------------------------------------------#
experiment_name = "First_try"       # Experiment name to define path were results are stored                           #
...
# Load data
dataset_info = load_dataset(norm='TPM', log2=True, corr_thr=coor_thr, p_thr=p_value_thr, force_compute=False,
...
for epoch in range(total_epochs):
    ...
    test_metrics = test(val_loader, model, device)
    ...
    if test_adversarial:
        ...
        adv_test_metrics = test(val_loader, model, device,
...
plot_training(train_metric_lst, test_metric_lst, adv_test_metric_lst, loss_list, train_performance_fig_path)
